...
source "qemu" "base-3-20-x86_64" {
  accelerator  = var.accelerator
  boot_command = [
    "<wait5>",
    "root<enter><wait>",
    "ifconfig eth0 up && udhcpc -i eth0<enter><wait5>",
    "wget http://{{ .HTTPIP }}:{{ .HTTPPort }}/${var.config_folder}alpine-linux-3-20/http/answers<enter><wait>",
  ...
  output_directory = "${var.output_dir}alpine-linux-3-20-x86_64"
  net_device       = var.net_device
  qemuargs         = [["-m", "${var.ram}M"], ["-smp", "${var.cpu}"]]
  shutdown_command = "poweroff"
  vm_name          = "alpine-linux-3-20-x86_64.qcow2"
...
build {
  ...
  provisioner "shell" {
    only             = ["qemu.base-3-19-x86_64"]
    valid_exit_codes = [0, 1]
    scripts          = ["./config/alpine-linux-3-19/files/setup.sh"]
  }

  provisioner "shell" {
    only             = ["qemu.base-3-20-x86_64"]
    valid_exit_codes = [0, 1]
    scripts          = ["./config/alpine-linux-3-20/files/setup.sh"]
  }
}