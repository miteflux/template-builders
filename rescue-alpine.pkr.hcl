...
source "qemu" "base-3-22-x86_64" {
  accelerator  = var.accelerator
  boot_command = [
    "<wait5>",
    "root<enter><wait>",
    "ifconfig eth0 up && udhcpc -i eth0<enter><wait5>",
    "wget http://{{ .HTTPIP }}:{{ .HTTPPort }}/${var.config_folder}rescue/alpine-linux-3-22/http/answers<enter><wait>",
...
build {
  ...
  provisioner "shell" {
    only             = ["qemu.base-3-22-x86_64"]
    valid_exit_codes = [0, 1]
    scripts          = ["./config/rescue/alpine-linux-3-22/files/setup.sh"]
  }
}