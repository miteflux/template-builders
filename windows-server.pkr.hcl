variables {
  ...
  windows_iso_2012r2_eval         = "./source/windows-server-2012-r2-eval.iso"
  windows_iso_2012r2_eval_checksum = "none"
  qemu_disk_cache                = "writeback"
  qemu_format                    = "qcow2"
}

source "qemu" "server-2012r2-standard-eval" {
  disk_size    = "14000"
  communicator = "winrm"
  floppy_files = [
    "./config/windows-shared/scripts/*",
    "./config/windows-server-2012r2-standard-eval/files/*",
    "./config/windows-shared/patches/cloudinit/windows.py"
  ]
  output_directory = "${var.output_dir}/windows-server-2012r2-standard-eval"
  qemuargs         = [
    ["-m", "${var.windows_memory}M"], ["-smp", var.windows_cpus],
    ["-drive", "file=${var.windows_iso_2012r2_eval},media=cdrom,index=2"],
    ["-drive", "file=${var.windows_virtio_driver},media=cdrom,index=3"], [
      "-drive",
      "file=${var.output_dir}/windows-server-2012r2-standard-eval/windows-server-2012r2-standard-eval.qcow2,if=virtio,cache=writeback,discard=ignore,format=qcow2,index=1"
    ]
  ]
  shutdown_command = "a:/sysprep.bat"
  vm_name          = "windows-server-2012r2-standard-eval.qcow2"
  disk_cache       = var.qemu_disk_cache
  accelerator      = var.accelerator
  headless         = var.headless
  iso_checksum     = var.windows_iso_2012r2_eval_checksum
  iso_urls         = [var.windows_iso_2012r2_eval]
...
source "qemu" "server-2025-standard" {
  ...
  accelerator      = var.accelerator
  headless         = var.headless
  iso_checksum     = var.windows_iso_2025_eval_checksum
  iso_urls         = [var.windows_iso_2025_eval]
  shutdown_timeout = var.shutdown_timeout
  format           = var.qemu_format
  vnc_bind_address = var.vnc_bind_address
  vnc_port_min     = var.vnc_port_min
